...
class FourthDigitScreen extends StatefulWidget{
  _FourthDigitScreen createState() => _FourthDigitScreen();
}

class _FourthDigitScreen extends State<FourthDigitScreen> with GameContainer{
  ...
  FocusNode _focusFirst, _focusSecond,_focusThird,_focusFourth;

  List<Widget> _listFourDigits = [];

  bool _isAdded;
  List<LotteryModel> _fourDigits = [];
  ...
  void initState() {
    ...
    _focusFirst = FocusNode();
    _focusSecond = FocusNode();
    _focusThird = new FocusNode();
    _focusFourth = new FocusNode();
    _isAdded = false;

    //For Three Digit
    _listFourDigits.add(numberField(_firstController,null,TextInputAction.next));
    _listFourDigits.add(numberField(_secondController,_focusSecond,TextInputAction.next));
    _listFourDigits.add(numberField(_thirdController,_focusThird,TextInputAction.next));
    _listFourDigits.add(numberField(_fourthController,_focusFourth,TextInputAction.done));
  }
...
//            }
//            else FocusScope.of(context).requestFocus(_focusSecond);
//          }
//          if(_secondController.text.isNotEmpty){
//            if(_thirdController.text.isNotEmpty){
//              FocusScope.of(context).unfocus();
//            }
//            else FocusScope.of(context).requestFocus(_focusThird);
//          }
//          if(_thirdController.text.isNotEmpty){
//            if(_fourthController.text.isNotEmpty){
//              FocusScope.of(context).unfocus();
//            }
//            else FocusScope.of(context).requestFocus(_focusFourth);
//          }
//          if(_fourthController.text.isNotEmpty){
//            FocusScope.of(context).unfocus();
//          }
          ...
          Stack(
            children: <Widget>[
              ...
              Positioned(
                ...
                child: Container(
                  alignment: Alignment.center,
                  child: _getNumberField(_listFourDigits.length),
                ),
              ),
              Positioned(
                top: 141,
  ...
  @override
  bool hasNumber(){
    if(_firstController.text.length > 0 && _secondController.text.length > 0  && _thirdController.text.length > 0  && _fourthController.text.length > 0){
      return true;
    }
    else return false;
  }
  ...
  void onAdded(){
    ...
    int fourth = int.parse(_fourthController.text.trim());
    setState(() {
      _isAdded = true;
      _fourDigits.add(new FourDigitModel(first,second,third,fourth,20));
    });
    clearController();
  }
  ...
  int  get calculateTotal{
    int _total = 0;
    for(LotteryModel lottery in _fourDigits){
                        ...
                        children: <Widget>[
                          ...
                          Container(
                            ...
                            child: Align(
                              alignment: Alignment.center,
                              child: Text(
                                _fourDigits[index].getPickedNumber,
                            ...
                            child: Row(
                              children: <Widget>[
                                Container(
                                  ...
                                  child: Text(
                                    _fourDigits[index].getAmount.toString() + ' SRY',
                                    style: TextStyle(fontSize: 15)
                                  ),
                                ),
                                Container(
                                  ...
                                  child: Tooltip(
                                    ...
                                    child: InkWell(
                                      onTap: (){
                                        setState(() {
                                          _fourDigits.removeAt(index);
                                        });
                                        if(_fourDigits.length == 0){
                                          setState(() {
                                            _isAdded = false;
                                          });
                                        }
